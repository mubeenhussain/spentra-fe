...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <main className="min-h-screen bg-[var(--background)] lg:grid lg:grid-cols-[1.05fr_.95fr]">
      <section className="relative hidden overflow-hidden bg-[var(--brand)] p-12 text-white lg:flex lg:flex-col">
        <div className="absolute -left-32 top-1/3 size-80 rounded-full bg-emerald-400/10 blur-3xl" />
        <div className="absolute -right-24 -top-24 size-80 rounded-full bg-[var(--accent)]/15 blur-3xl" />
        ...
        <div className="relative my-auto max-w-xl">
          ...
          <h2 className="text-5xl font-semibold leading-[1.12] tracking-tight">
            Know where it goes.
            <br />
            <span className="text-[var(--accent)]">Keep more of it.</span>
          </h2>
      ...
      <section className="flex min-h-screen flex-col px-6 py-7 sm:px-12 lg:px-16 xl:px-24">
        <div className="mb-12 text-lg text-slate-950 lg:hidden">
          <BrandMark />
        </div>
...
function Stat({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/7 p-5 backdrop-blur">
      <p className={`text-2xl font-semibold ${accent ? "text-emerald-300" : "text-[var(--accent)]"}`}>
        {value}
      </p>
