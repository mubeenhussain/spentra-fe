...
export function Logo({ size = "md" }: { size?: "sm" | "md" }) {
  ...
  return (
    <span className={`grid place-items-center rounded-xl bg-brand-soft text-brand-text ${box}`}>
...
export function BrandMark({ size = "md", href = "/" }: { size?: "sm" | "md"; href?: string }) {
  return (
    <Link href={href} className="flex items-center gap-2.5 font-bold text-heading">
      <Logo size={size} />
      Spentra
    </Link>
  );
